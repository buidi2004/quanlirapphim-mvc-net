...
> **Yêu cầu bắt buộc:** hiệu ứng kính trên Android phải là **blur thật** (native gaussian blur), được canh chỉnh
> thông số (radius, tint, saturation, viền sáng) để nhìn **giống hệt Apple Liquid Glass trên iOS** — không chấp
> nhận kiểu "giả lập" bằng một lớp `View` bán trong suốt (`rgba(...)`) không có blur, vì mắt người phân biệt được
> ngay và phá vỡ cảm giác cao cấp của toàn bộ app.

### 1. Floating Dock Tab Bar (macOS Dock style)

Bottom Tab Bar của app PHẢI được thiết kế theo phong cách **macOS Dock** — nổi (floating), bo cong tròn, hiệu ứng kính mờ, và **phải trông giống nhau tuyệt đối trên iOS lẫn Android**:
...
- Nền kính mờ: dùng `GlassSurface` (component dùng chung, xem mục 2) — tự chọn engine blur đúng theo platform, KHÔNG rẽ nhánh thủ công trong từng màn hình
- Viền mỏng: `borderWidth: 0.5, borderColor: 'rgba(255,255,255,0.12)'` (light: `rgba(0,0,0,0.08)`)
- Viền sáng hắt trên cùng (top highlight 1px, `rgba(255,255,255,0.35)`) để giả lập ánh sáng phản chiếu trên mặt kính — chi tiết này quyết định độ "giống Apple" nhiều nhất, không được bỏ qua trên Android
- Shadow nhẹ phía dưới tạo cảm giác floating
- Nút "Đặt vé" giữa dock nổi lên (`marginTop: -18`) với background accent, KHÔNG áp glass cho nút này (nút CTA cần đặc, tương phản mạnh)

**Code pattern (Custom tabBar, dùng `GlassSurface` chung cho cả 2 nền tảng):**
```tsx
...
### 2. Glassmorphism Standards — Blur thật 100% trên cả iOS và Android

`expo-glass-effect` (`GlassView`) chỉ chạy native Liquid Glass **thật** trên iOS 26+. Trên mọi phiên bản iOS cũ hơn và **trên toàn bộ Android**, `GlassView` tự fallback về một `View` trong suốt phẳng — nhìn **không giống Apple chút nào** (thiếu độ mờ/blur thật, thiếu độ sâu). Vì yêu cầu là hiệu ứng phải giống Apple 100% trên cả Android, **không được dừng ở fallback mặc định này**.

**Giải pháp bắt buộc: dùng `expo-blur` (`BlurView`) làm engine blur chính cho mọi trường hợp không phải iOS 26+ Liquid Glass.**
`BlurView` sử dụng blur gaussian thật ở tầng native (UIVisualEffectView trên iOS, RenderEffect/RenderScript-backed blur trên Android — hỗ trợ tốt từ Android 12+, có shim cho version cũ hơn), cho ảnh nhìn xuyên qua bị làm mờ thật sự — đây là yếu tố quan trọng nhất để "giống Apple", quan trọng hơn nhiều so với chọn đúng thư viện.

**Nguyên tắc chọn engine theo thứ tự ưu tiên (được đóng gói sẵn trong component `GlassSurface`, KHÔNG viết lại logic này ở từng màn hình):**

| Điều kiện | Engine | Ghi chú |
|-----------|--------|---------|
| iOS ≥ 26, `isGlassEffectAPIAvailable()` = true | `GlassView` (`expo-glass-effect`) | Liquid Glass thật của Apple |
| iOS < 26 | `BlurView` (`expo-blur`), `tint="systemMaterialDark"`/`"systemMaterialLight"` | Blur thật, tint hệ thống giống iOS cũ |
| Android (mọi version được support) | `BlurView` (`expo-blur`), `tint="dark"`/`"light"`, `experimentalBlurMethod="dimezisBlurView"` | Bật blur method thật trên Android thay vì blur giả lập mặc định của RN |

**Component dùng chung — `src/components/ui/GlassSurface.tsx`:**
```tsx
...
type GlassVariant = 'dock' | 'header' | 'card' | 'sheet';
...
const GLASS_TUNING: Record<GlassVariant, { intensity: number; iosTint: string; androidTint: 'light' | 'dark' }> = {
  dock:   { intensity: 62, iosTint: 'systemMaterialDark', androidTint: 'dark' },
  header: { intensity: 45, iosTint: 'systemMaterialDark', androidTint: 'dark' },
  card:   { intensity: 38, iosTint: 'systemMaterialDark', androidTint: 'dark' },
  sheet:  { intensity: 70, iosTint: 'systemMaterialDark', androidTint: 'dark' },
};
...
export const GlassSurface = ({ variant, borderRadius = Theme.radius.card, style, children }: GlassSurfaceProps) => {
  ...
  return (
    <BlurView
      ...
      tint={Platform.OS === 'ios' ? (tuning.iosTint as any) : tuning.androidTint}
      // Trên Android, bật engine blur thật (dimezisBlurView) thay vì bản mô phỏng mặc định
...
**Vì sao cách này cho kết quả "giống Apple 100%" trên Android:**
- `BlurView` với `experimentalBlurMethod="dimezisBlurView"` dùng blur GPU thật (RenderEffect trên Android 12+, fallback renderscript) — không phải overlay `rgba` tĩnh, nên nội dung phía sau (poster phim, ảnh nền) thật sự bị làm nhòe khi cuộn qua, y hệt hiệu ứng UIVisualEffectView của Apple.
- Thông số `intensity` trong bảng `GLASS_TUNING` được canh chỉnh riêng cho từng loại surface (dock cần mờ nhiều hơn header, card cần mờ ít hơn để không che nội dung) — KHÔNG dùng một giá trị `intensity` chung cho tất cả.
- Viền `topHighlight` (1px trắng mờ ở cạnh trên) là chi tiết nhỏ nhưng chính là thứ mắt người nhận diện là "kính" thay vì "nhựa mờ" — bắt buộc có ở cả 2 nền tảng.
- Test bắt buộc trên **thiết bị Android thật** (không chỉ emulator) vì blur GPU có thể khác nhau giữa các dòng máy; nếu máy quá cũ không hỗ trợ `dimezisBlurView`, `expo-blur` tự fallback về blur phần mềm — vẫn là blur thật, chỉ chậm hơn, KHÔNG bao giờ fallback về overlay phẳng không blur.

**Glass Color Tokens (`Theme.colors.glass`):**
```typescript
glass: {
  background: 'rgba(18, 18, 30, 0.65)',       // Nền fallback khi cần 1 lớp overlay tĩnh phía sau BlurView
  backgroundLight: 'rgba(26, 26, 46, 0.55)',   // Nền nhạt hơn
  border: 'rgba(255, 255, 255, 0.12)',          // Viền glass chính (dark)
  borderLightMode: 'rgba(0, 0, 0, 0.08)',       // Viền glass chính (light)
  topHighlight: 'rgba(255, 255, 255, 0.35)',    // Viền sáng hắt trên cùng
  tint: 'rgba(13, 13, 13, 0.4)',               // Tint color cho GlassView (iOS 26+)
}
```

**Bề mặt dùng Glass (map surface → variant của `GlassSurface`):**

| Surface | `GlassSurface` variant | Khi nào dùng |
|---------|-------------------------|-------------|
| **Bottom Tab Bar (Dock)** | `dock` | Luôn luôn |
| **Screen Header** | `header` | Khi cần header trong suốt (scroll-through) |
| **Sticky Bottom Bar** | `sheet` | SeatSelection, Concession, Payment |
| **Cards quan trọng** | `card` | Payment summary card, order summary |
| **Modals / BottomSheet** | `sheet` | Popup, BottomSheet nền |

**Reusable Components (đều build trên `GlassSurface`, không tự viết BlurView/GlassView riêng lẻ ở từng screen):**

#### GlassCard (`src/components/ui/GlassCard.tsx`)
```tsx
import { GlassSurface } from './GlassSurface';

export const GlassCard = ({ children, bordered = true, borderRadius = Theme.radius.card, style }: Props) => (
  <GlassSurface variant="card" borderRadius={borderRadius} style={[bordered && styles.bordered, style]}>
    {children}
  </GlassSurface>
);
```
...
/>
```
Header iOS-style blur, dùng `GlassSurface variant="header"` bên trong. Auto SafeArea padding top. Hoạt động giống hệt trên Android nhờ `BlurView`.

**Quy tắc Glass:**
- ❌ KHÔNG dùng glass cho tất cả mọi thứ — chỉ dùng cho surface "nổi" hoặc "floating"
- ❌ KHÔNG lồng `GlassSurface` bên trong `GlassSurface` (nếu cần group nhiều mảnh kính liền kề trên iOS 26+, dùng `GlassContainer` của `expo-glass-effect` bên trong nhánh `canUseNativeLiquidGlass`)
- ❌ KHÔNG bao giờ thay `BlurView` bằng một `View` có `backgroundColor: 'rgba(...)'` tĩnh để "tiết kiệm hiệu năng" — đây chính là lỗi khiến Android trông khác iOS, không được chấp nhận trong app này
- ✅ Luôn dùng component `GlassSurface` chung, không viết logic chọn platform lặp lại ở từng màn hình
- ✅ Viền glass luôn mỏng: `borderWidth: 0.5` (KHÔNG dùng 1 hoặc 2)
- ✅ Luôn có `topHighlight` 1px ở cạnh trên mỗi mảnh kính
- ✅ Test song song iOS và Android trên thiết bị thật trước khi merge, so sánh trực tiếp side-by-side để đảm bảo độ mờ và độ sáng viền lệch nhau không quá nhận biết được bằng mắt
...
| Component | Mô tả | Dùng ở |
|-----------|--------|--------|
| `GlassSurface` | Component nền tảng chọn engine blur đúng theo platform (GlassView iOS 26+ / BlurView mọi trường hợp khác) | Toàn bộ các component glass khác build trên nó |
| `GlassCard` | Card kính mờ iOS (build trên `GlassSurface`) | B5, B4, B3, Cards quan trọng |
| `GlassHeader` | Header blur iOS-style (build trên `GlassSurface`) | Tất cả stack screens |
| `FloatingDockTabBar` | Custom tab bar kiểu macOS Dock (build trên `GlassSurface`) | AppNavigator (Tab.Navigator) |
...
### Glassmorphism (Android == iOS)
- [ ] Mọi surface kính đều dùng chung component `GlassSurface`, không có `View` với `rgba(...)` giả lập blur ở bất kỳ đâu?
- [ ] Trên Android, `experimentalBlurMethod="dimezisBlurView"` đã bật và blur nhìn thấy rõ nội dung phía sau bị làm nhòe (không phải chỉ tối màu đi)?
- [ ] Đã test trên tối thiểu 1 thiết bị Android thật (không chỉ emulator)?
- [ ] Đặt ảnh chụp màn hình Dock/Header trên iOS và Android cạnh nhau — độ mờ, tint, viền sáng có lệch nhau rõ bằng mắt thường không? Nếu có, chỉnh lại `GLASS_TUNING`.
- [ ] `topHighlight` 1px có xuất hiện trên cả 2 nền tảng?
...
| Expo Glass Effect | Native Liquid Glass (iOS 26+ only) | `expo-glass-effect` |
| Expo Blur | Blur thật cross-platform (engine chính cho Android + iOS < 26) | `expo-blur` |
