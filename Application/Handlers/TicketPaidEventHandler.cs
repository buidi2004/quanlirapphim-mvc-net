...
public class TicketPaidEventHandler(IDbConnection db) : INotificationHandler<TicketPaidEvent>
{
    public async Task Handle(TicketPaidEvent notification, CancellationToken cancellationToken)
    {
        ...
        var currentSpent = await db.ExecuteScalarAsync<double>("SELECT total_spent FROM users WHERE id = @UserId", new { UserId = notification.UserId });
        ...
        {
            await db.ExecuteAsync("UPDATE users SET member_level = @Tier WHERE id = @UserId", new { Tier = newTier, UserId = notification.UserId });
        }
    }
}
