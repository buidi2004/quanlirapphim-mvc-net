    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} bounces={false}>
        ...
        <View style={styles.heroContainer}>
          <Image source={{ uri: cinema.imageUrl }} style={styles.heroImage} contentFit="cover" />
          <LinearGradient
            colors={['transparent', 'rgba(0,0,0,0.5)', Theme.colors.background]}
            style={styles.heroGradient}
          >
            <View style={styles.heroCityBadge}>
              <Text style={styles.heroCityText}>{cinema.city}</Text>
            </View>
        ...
        <View style={styles.body}>
          ...
          <View style={styles.section}>
            ...
            {/* Showtimes */}
            {MOCK_SHOWTIMES_TODAY.map(movie => (
              <View key={movie.id} style={styles.showtimeCard}>
                <View style={styles.showtimeMovieRow}>
                  <Text style={styles.showtimeMovieTitle} numberOfLines={1}>{movie.title}</Text>
                  ...
                  <Text style={styles.showtimeDuration}>{movie.duration}ph</Text>
                </View>
                <View style={styles.timesRow}>
                  {movie.times.map(t => (
                    <TouchableOpacity
                      ...
                      onPress={() => navigation.navigate('SeatSelection', { showTime: t, movieTitle: movie.title })}
                    >
...
const styles = StyleSheet.create({
  ...
  heroCityText: { color: '#fff', fontSize: 11, fontWeight: 'bold' },
  heroTitle: { color: '#fff', fontSize: 24, fontWeight: '800', marginBottom: 6 },
  heroAddress: { color: '#ccc', fontSize: 13, lineHeight: 18 },
  ...
  infoValue: { color: '#fff', fontSize: 11, fontWeight: 'bold', textAlign: 'center' },
  infoLabel: { color: '#666', fontSize: 10, textAlign: 'center' },
  ...
  facilityText: { color: '#ccc', fontSize: 12 },
  ...
  expDesc: { color: '#666', fontSize: 10, textAlign: 'center', lineHeight: 14 },
  ...
  dateDayName: { color: '#666', fontSize: 10, marginBottom: 2 },
  dateDayNameActive: { color: Theme.colors.warning },
  dateDayNum: { color: '#fff', fontSize: 18, fontWeight: 'bold' },
  dateDayNumActive: { color: Theme.colors.warning },
  dateMonth: { color: '#555', fontSize: 10 },
  ...
  showtimeMovieTitle: { color: '#fff', fontSize: 14, fontWeight: 'bold', flex: 1 },
  ratingBadge: { paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 },
  ratingBadgeText: { color: '#fff', fontSize: 10, fontWeight: 'bold' },
  showtimeDuration: { color: '#888', fontSize: 12 },
