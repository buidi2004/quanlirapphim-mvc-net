...
echo "=========================================="
echo "🎬 Khởi chạy hệ thống CinemaX 🎬"
echo "=========================================="
...
EOL
    ...
    echo "🐳 Đang build và khởi chạy toàn bộ Docker (Web, DB, Mobile)..."
    docker compose up --build -d
else
    echo "⚡ Chế độ tối ưu: Khởi chạy Web (.NET) & Database (MySQL) - (Không chạy Mobile)"
    echo "🐳 Đang build và khởi chạy Docker (Web, DB)..."
...
if [ "$1" == "mobile" ] || [ "$1" == "all" ]; then
    ...
    echo "   docker compose attach mobile"
else
    echo "💡 Muốn chạy thêm Mobile Expo? Dùng: ./start-dev.sh mobile"
fi
echo "=========================================="
